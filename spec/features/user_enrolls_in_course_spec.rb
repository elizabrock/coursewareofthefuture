...
feature "Student enrolls in course" do
  ...
  scenario "Fix: Instructors are automatically enrolled in courses" do
    ...
    expect(current_path).to eql course_path(cohort4a)
    click_link "Inquizator"
    click_link "Cohort 4a"
    expect(current_path).to eql course_path(cohort4a)
    click_link "Inquizator"
    click_link "Cohort 4b"
    expect(current_path).to eql course_path(cohort4b)
  end

  scenario "Student enrolling in a course" do
    signin_as :student
    visit root_path
    expect(page).to have_content "Please select a course below to join it"
    expect(page).to have_button("Join Cohort 4a")
    expect(page).to have_button("Join Cohort 4b")
    expect(page).not_to have_button("Join Cohort 1a")
    expect(page).not_to have_button("Join Cohort 1b")
    click_button "Join Cohort 4a"
    expect(page).to have_content "You are now enrolled in Cohort 4a"
    expect(current_path).to eql course_path(cohort4a)
  end
  ...
  scenario "Student signing in or going to the homepage takes you to your course" do
    ...
    visit root_path
    expect(current_path).to eql course_path(cohort3)
  end
  ...
  scenario "Student signing up for a second course" do
    ...
    expect(page).not_to have_button("Join Cohort 1a")
    expect(page).not_to have_button("Join Cohort 1b")
    expect(page).not_to have_button("Join Cohort 3")
    click_button "Join Cohort 4a"
    expect(page).to have_content "You are now enrolled in Cohort 4a"
    expect(current_path).to eql course_path(cohort4a)
  end
  ...
  scenario "Signing up for a second course makes it your default course" do
    ...
    expect(page).to have_content "You are now enrolled in Cohort 4b"
    expect(current_path).to eql course_path(cohort4b)
    visit root_path
    expect(current_path).to eql course_path(cohort4b)
  end
  ...
  scenario "Enroll in another course doesn't show up if there are no courses" do
    ...
    expect(page).not_to have_content("Please select a course")
    expect(page).to have_content("There are no open courses at this time.")
  end
  ...
  scenario "If there are no courses, enrollment page shows appropriate message" do
    ...
    signin_as :student
    visit root_path
    expect(page).not_to have_link "Enroll in another course"
  end
  ...
  scenario "Enrollments view shows all students enrolled in course." do
    ...
    expected_list.each_with_index do |content, row|
      ...
      expect(page).to have_xpath("//ul/li[#{row+1}][contains(normalize-space(.), '#{text}')]//img[contains(@src, '#{image_path}')]")
    end
  end
end
