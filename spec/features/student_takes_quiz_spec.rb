...
feature "Student takes quiz" do
  ...
  scenario "Answering some questions without submitting" do
    ...
    expect(page).to have_content "Your quiz progress has been saved"
    expect(current_path).to eql course_assignments_path(course)
    expect(page).to have_content "Baseline Knowledge (incomplete, due"
    click_link "Baseline Knowledge"
    within(div_labeled("Are you happy?")){ expect(page).to have_checked_field("True") }
    within(div_labeled("Is class over?")){ expect(page).to have_checked_field("False") }
    expect(find_field("What are you happy about?").value).to match /#{""}/
  end
  ...
  scenario "Submitting completed quiz" do
    ...
    expect(current_path).to eql course_assignments_path(course)
    expect(page).to have_content "Baseline Knowledge (submitted for grading)"
  end
  ...
  scenario "Student can't submit partial quiz" do
    ...
    page.find(:xpath, "//div[contains(normalize-space(.), '#{"Is class over?"}')]/label[contains(text(), '#{"False"}')]/input").set(true)
    click_button "Finish Quiz"
    expect(page).to have_content "Your quiz cannot yet be submitted for grading"
    within(div_labeled("Are you happy?")){ expect(page).to have_checked_field("True") }
    within(div_labeled("Is class over?")){ expect(page).to have_checked_field("False") }
    expect(page).to have_error_message("can't be blank", on: "What are you happy about?")
    visit course_assignments_path(course)
    expect(page).to have_content "Baseline Knowledge (incomplete"
  end
