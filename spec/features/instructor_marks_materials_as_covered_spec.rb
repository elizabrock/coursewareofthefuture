...
feature "Instructor marks materials as covered", vcr: true do
  ...
  scenario "Students don't see 'Mark as Covered'" do
    ...
    expect(page).not_to have_content "Mark as Covered"
  end
  ...
  scenario "Instructor marks item as covered, date defaults to today" do
    Timecop.travel(Time.new(2013, 03, 12)) do
      ...
      click_link "Materials"
      within_tr_for("Logic"){ expect(page).not_to have_content("Covered") }
      mark_as_covered("Logic")
      expect(page).to have_content "Logic has been marked as covered on 3/12."
      within_tr_for("Logic"){ expect(page).to have_content("Covered") }
      mark_as_covered("Basic Control Structures")
      expect(page).to have_content "Basic Control Structures has been marked as covered on 3/12."
      within_tr_for("Logic"){ expect(page).to have_content("Covered") }
      within_tr_for("Basic Control Structures"){ expect(page).to have_content("Covered") }
    end
  end
  ...
  scenario "Instructor marks item as covered on a particular date" do
    ...
    expect(page).to have_content "Basic Control Structures has been marked as covered"
    within_tr_for("Logic"){ expect(page).to have_content("Covered") }
    within_tr_for("Basic Control Structures"){ expect(page).to have_content("Covered") }
  end
  ...
  scenario "Instructor changes the date an item was covered" do
    Timecop.travel(Time.new(2013, 03, 13)) do
      ...
      within_tr_for("Logic") do
        expect(page).not_to have_content "Covered"
      end
      mark_as_covered("Logic")
      expect(page).to have_content "Logic has been marked as covered on 3/13."
      within_tr_for("Logic") do
        expect(page).to have_content "Covered 3/13"
      end
    end
    ...
    Timecop.travel(Time.new(2013, 03, 12)) do
      ...
      expect(page).to have_content "Logic has been marked as covered on 3/11."
      within_tr_for("Logic"){ expect(page).to have_content("Covered") }
      within_tr_for("Basic Control Structures"){ expect(page).to have_content("Covered") }
    end
  end
  ...
  scenario "Marking an item as covered in one class does not change it's status in other classes" do
    ...
    within_tr_for("Logic"){ expect(page).not_to have_content("Covered") }
    mark_as_covered("Logic")
    expect(page).to have_content "Logic has been marked as covered"
    within_tr_for("Logic"){ expect(page).to have_content("Covered") }
