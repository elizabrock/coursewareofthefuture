...
feature "Instructor creates quiz", vcr: true do
  ...
  scenario "Creating the initial quiz" do
    ...
    expect(page).to have_content "Your quiz has been created. Add questions and then publish it."
    click_link "Assignments"
    expect(page).to have_content "Baseline Knowledge Quiz (unpublished)"
  end
  ...
  scenario "Free Text Questions" do
    ...
    expect(page).to have_content "Add another question"
    ...
    expect(page).to have_content "Your quiz has been updated."
    within(page.find(:xpath, "//fieldset[position()=2]")) do
     expect(page).to have_content "Have you installed ruby 2.1?"
     expect(page).to have_content "I am a little teapot"
    end
    expect(page).to have_content "Add another question"
    within(page.find(:xpath, "//fieldset[position()=2]")) do
    ...
    expect(page).to have_content "Your quiz has been updated"
    within(page.find(:xpath, "//fieldset[position()=2]")) do
      expect(page).to have_content "Which ruby version are you running?"
      expect(page).to have_content "ruby 2.1.1-p76"
    end
    expect(page).to have_content "Add another question"
  end
  ...
  scenario "True/False Questions" do
    ...
    expect(page).to have_content "Your quiz has been updated"
    within(page.find(:xpath, "//fieldset[position()=2]")) do
      expect(page).to have_content "Have you installed ruby 2.0?"
  ...
  scenario "Editing quizzes" do
    ...
    click_button "Save Changes"
    expect(page).to have_content "Your quiz has been updated."
    expect(Question.where(question_type: "boolean",
                   question: "Are you satisfied?",
                   correct_answer: "true").count).to eql 1
    expect(Question.where(question_type: "free_text",
                   question: "What are you happy about?",
                   correct_answer: "There can't be a correct answer.").count).to eql 1
    expect(Question.where(question_type: "boolean",
                   question: "Is class over?",
                   correct_answer: "True").count).to eql 1
  end
  ...
  scenario "Validations" do
    ...
    fill_in "Title", with: ""
    click_button "Create Quiz"
    expect(page).to have_content "Your quiz could not be saved."
    ...
    click_button "Save Changes"
    expect(page).to have_content "Your quiz could not be updated"
    expect(page).to have_error_message("can't be blank", on: "Question")
    expect(find_field("Correct Answer").value).to match /#{"Blank"}/

    field_id = find(:field, "Question Type")[:id]
    expect(page).to have_xpath "//select[@id = '#{field_id}']/option[@selected and text() = 'Free Text']"
  end
  ...
  scenario "Publishing quizzes" do
    ...
    click_link "Assignments"
    click_link "Final Checkin"
    expect(page).not_to have_content "Setting a deadline will publish this quiz."
    fill_in "Deadline", with: "2014/05/14"
    click_button "Save Changes"
    expect(page).not_to have_content "Your quiz has been published."
    visit course_assignments_path(course)
    expect(page).to have_content "Final Checkin (unpublished, due 5/14)"

    click_link "Sign Out"
    signin_as :student, courses: [course]
    click_link "Assignments"
    expect(page).not_to have_content "Final Checkin"
    ...
    expect(page).to have_content "Final Checkin (due 5/14)"
