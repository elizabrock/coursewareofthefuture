RSpec::Matchers.define :have_calendar_entry do |expected, options|
  match do |page|
    ...
    expect(page).to have_css("td[data-date='#{date}'] span", text: message)
  end
end

RSpec::Matchers.define :have_exercises_from_github do
  match do |page|
    expect(page).not_to have_content("Exercises")
    expect(page).not_to have_content("Ruby Koans")
  end
end
...
RSpec::Matchers.define :have_options_for do |field, options|
  match do |page|
    ...
    expect(actual_options).to eql expected_options
  end
...
RSpec::Matchers.define :have_checkboxes do |expected_checkboxes|
  match do |page|
    ...
    expect(@actual_checkboxes).to eql expected_checkboxes
  end
...
RSpec::Matchers.define :have_list do |contents|
  match do |page|
    contents.each_with_index do |text, row|
      expect(page).to have_xpath("//*/li[#{row+1}][contains(normalize-space(.), '#{text}')]")
    end
  end
end
