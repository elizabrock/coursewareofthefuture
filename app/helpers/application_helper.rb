module ApplicationHelper
  ...
  def link_to_course(course, options = {})
    full_title = "#{course.title} (#{course.start_date} - #{course.end_date})"
    if can?(:manage, course) and !current_user.courses.include?(course)
      options[:method] = :post
      link_to full_title, main_app.course_enrollments_path(course), options
    else
      link_to full_title, main_app.course_path(course), options
    end
  end
